...
namespace SDroid.SteamMobile
{
    ...
    public static class SteamTime
    {
        ...
        public static async Task<bool> ReAlignTime()
        {
            try
            {
                ...
                var serverResponse = await SteamWebAPI.Default
                    .RequestObject<SteamWebAPIResponse<QueryTimeResponse>>(
                        "ITwoFactorService",
                ...
                if (serverResponse?.Response?.ServerTime == null)
                {
                    return false;
                }
        ...
        {
            return (long) Math.Floor((dateTime.ToUniversalTime() - Epoch).TotalSeconds);
        }
    }
}